...
public class PipeLayerRenderer implements LayerRenderer {
    ...
    public PipeLayerRenderer(BuildingRegistry registry, TextureLoad textures) {
    ...
    public void render(SpriteBatch batch, VisibleBounds bounds, float stateTime) {
        ...
        for (int x = bounds.startX(); x < bounds.endX(); x++) {
            for (int y = bounds.startY(); y < bounds.endY(); y++) {
                ...
                if (current == null || !renderedBuildingsThisFrame.add(current)) {
                    continue;
                }
